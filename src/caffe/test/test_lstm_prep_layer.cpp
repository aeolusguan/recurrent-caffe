...
class LSTMPrepLayerTest: public MultiDeviceTest<TypeParam> {
 ...
 protected:
  LSTMPrepLayerTest() :
    ...
    std::vector<int> x_input_shape(2);
    x_input_shape[0] = 2;
    x_input_shape[1] = 100;
...
TYPED_TEST(LSTMPrepLayerTest, TestSetUp) {
  ...
  layer_param.mutable_lstm_prep_param()->set_tile_k(10);
  ...
  EXPECT_EQ(this->stacked_x_blob_->num_axes(), 3);
  EXPECT_EQ(this->stacked_x_blob_->shape()[0], 10);
  EXPECT_EQ(this->stacked_x_blob_->shape()[1], 2);
  EXPECT_EQ(this->stacked_x_blob_->shape()[2], 100);
}
